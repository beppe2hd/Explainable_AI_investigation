% The funcion generate and save as CSV a set of features vector made up by
% gaussian random variables. Then the contribution on specific features is
% added 
...
%%% Train matrix %%%%
train_mat=noise_factor*rand(num_of_element_train,vector_size+1); % matrix generatione
...
gt_class_0 = zeros(num_of_element_train/2,1); % gt for features 0
gt_class_1 = ones(num_of_element_train/2,1); % gt for features 1

% the features contribution are added to the main matrix
train_mat(1:num_of_element_train/2,class_0_features_start:class_0_features_end) = train_mat(1:num_of_element_train/2,class_0_features_start:class_0_features_end)+class_0;
train_mat(num_of_element_train/2+1:end,class_1_features_start:class_1_features_end)=train_mat(num_of_element_train/2+1:end,class_1_features_start:class_1_features_end)+class_1;
...
gt_class_0 = zeros(num_of_element_val/2,1); % gt for features 0
gt_class_1 = ones(num_of_element_val/2,1); % gt for features 1

% the features contribution are added to the main matrix
val_mat(1:num_of_element_val/2,class_0_features_start:class_0_features_end) = val_mat(1:num_of_element_val/2,class_0_features_start:class_0_features_end)+class_0;
val_mat(num_of_element_val/2+1:end,class_1_features_start:class_1_features_end)=val_mat(num_of_element_val/2+1:end,class_1_features_start:class_1_features_end)+class_1;
